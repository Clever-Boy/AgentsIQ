...
class Agent:
    ...
    def act(self,task,router,tools=None):
        ...
        # Only use tools when explicitly requested via "use:<tool>"
        ...
        entry={"agent":self.name,"role":self.role,"task":task,"response":response,
               "model_used":model_to_use if tool_output is None else f"tool:{used_tool}",
               "confidence":conf,"latency":time.time()-start,"id":str(uuid.uuid4())}
